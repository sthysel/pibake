...
from pibake.utils import (get_default_image_source, get_filename_from_response,
                          get_filesize, get_image_file, get_image_source_list,
                          get_images, remove_existing_file)

from . import disks, settings
...
@click.option(
    ...
    default=0,
    count=True,
    help='Level of verbosity of logs',
)
@click.option(
    '-c',
    '--cache-path',
...
def fetch(config, image, overwrite):
    ...
    click.echo(f'Fetching {cache_file_name}')
...
def list_images(config):
    ...
    if sds:
        ...
        for disk in sds:
            click.echo('Device {device} mounted on {mountpoint}'.format(
                **disk._asdict()))
    else:
        click.echo(
            'No mounted SD cards found. Insert a SD card and format it to vfat.'
        )
...
def bake_image(config, image, mountpoint):
    ...
    if image not in valid_images:
        click.echo('Please pick a valid image, current choices are: {}'.format(
            ', '.join(valid_images)))
        sys.exit()

    valid_mounts = [disk.mountpoint for disk in disks.get_mounted_candidates()]
    if mountpoint not in valid_mounts:
        click.echo('Please pick a valid mountpoint, one of {}'.format(
            ', '.join(valid_mounts)))
        sys.exit()
